...
module.exports = {
    ...
    async execute (interaction, client, Discord) {
        ...
        let notifmsg = await interaction.channel.send({content: "How much would you like to withdraw? ||<amount>||" })
        let amount;
        collector.on("collect", (m) => {
          if (isNaN(m.content)) {
            return interaction.channel.send({ content: "*Don't forget to give me a withdrawl amount!*" });
          } else {
            amount = m.content;
            notifmsg.delete()
            m.delete()
        ...
        collector.on("end", async (collected) => {
          if (collected.size == 0) {
            let timemsg = await interaction.channel.send({
              content: "You didn't provide a withdrawl amount in time! Try again.",
              ephemeral: true,
            });
            setTimeout(() => {
              timemsg.delete();
            ...
            if (amount > memberBal.bank) {
              let failmsg = await interaction.channel.send(
                `You're trying to withdraw more coins than you have in your account!`
              );
              setTimeout(() => {
                failmsg.delete();
              }, 5000);
            ...
            let finmsg = await interaction.channel.send(`Successfully made a withdrawl of ${amount} coins, you're now holding ${newbal} coins.`);
            setTimeout(() => {
                finmsg.delete();
              }, 5000);
              return;
