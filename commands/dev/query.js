module.exports = {
    ...
    async execute(client, message, args, Discord) {
        ...
        if(!args[1] || args[1] === "all") {
            ...
            const econ = await client.functions.get("getAuthorEcon").execute(message);
            ...
            message.channel.send(`Mass query took ${total}ms to complete.`)
        }
        if(args[1] === "econ") {
            const start = Date.now()
            const settings = await client.functions.get("getAuthorEcon").execute(message);
            const end = Date.now()
            const total = end - start
        ...
        if(args[1] === "util") {
            const start = Date.now()
            const data = await client.functions.get("getUtil").execute(message);
            const end = Date.now()
            const total = end - start
